...
class ExampleStateManager(Manager):
    ...
    def measure_member_progress(self, examples, members):
        done_count = (
            self.filter(example_id__in=examples).values("confirmed_by__email").annotate(total=Count("confirmed_by"))
        )
        response = {
            "total": examples.count(),
            "progress": [{"user": obj["confirmed_by__email"], "done": obj["total"]} for obj in done_count],
        }
        members_with_progress = {o["confirmed_by__email"] for o in done_count}
        for member in members:
            if member.email not in members_with_progress:
                response["progress"].append({"user": member.email, "done": 0})
        return response
