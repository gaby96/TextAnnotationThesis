...
# This class will be called with the model_name, labels object, and the count of labels in the object
class BERTSequenceLabeling:
    def __init__(self, num_labels, label_map, model_name='bert-base-uncased'):
    ...
    def tokenize_and_preserve_labels_offsets(self, text, labels):
        ...
        for line in text.splitlines():  # Process each line separately
            if line.strip():  # Only process non-empty lines
                ...
                # Add a newline character after processing a line
                current_position += 1
        
        return tokens, labels_preserved, offsets  # Return the processed tokens, labels, and offsets
    ...
    def predict_labels(self, sentence, labels):
        ...
        # Map the predicted label IDs back to a dictionary with id and text
        predicted_labels = [{'id': label_id, 'text': self.label_map[label_id]['text']} for label_id in predicted_label_ids]

        # Align labels with their corresponding offsets
        results = []
        for token, label, (start, end) in zip(tokens, predicted_labels, offsets):
            if label['text'] != "O":  # Filter out non-entity labels
                results.append({
                    "token": token,
                    "label": label['text'],  # Use only the text from the label
                    "start_offset": start,
                    "end_offset": end
                })
...
# labels = ["B-PER", "O", "O", "B-LOC", "I-LOC", "I-LOC"]  # Adjust labels to standard NER labels


# sequence_labeler = BERTSequenceLabeling(num_labels=5)
# results = sequence_labeler.predict_labels(sentence, labels)
