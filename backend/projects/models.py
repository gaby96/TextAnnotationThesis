...
#PolymorphicModel is a base class provided by django-polymorphic. 
#When you create Django models that inherit from PolymorphicModel, 
#Django can store and retrieve objects of subclasses through a common superclass query.
#This means you can query the superclass model and get instances of the subclass models, 
#with each instance automatically being the correct subclass.
...
class Project(PolymorphicModel):
    ...
    #The @property decorator allows the method to be called without the paranthesis, just like an attribute
    #The @abc.abstractmethod decorator is used to declare a method as an abstract method within an abstract base class.
    @property
    @abc.abstractmethod
    def is_text_project(self) -> bool:
        return False
    
    #he method starts by creating a copy of the current project instance. 
    #It does this by fetching the project from the database (Project.objects.get(pk=self.pk)) 
    #and then setting its pk (primary key) and id to None. This ensures that when save() is called, 
    #a new instance is created in the database rather than updating the existing one.
