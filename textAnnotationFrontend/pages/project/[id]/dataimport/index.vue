                ...
                <EasyDataTable :headers="headers" :items="items">
                    ...
                    <template #item-action="item">
                        <div class=" text-center flex justify-left items-center gap-2">
                            ...
                            <a href="#" @click.prevent="editText(item.id, item.uuid)">
                                <svg xmlns="http://www.w3.org/2000/svg"
                            ...
                            <!-- Delete Icon -->
                            <a href="#" @click.prevent="deleteExampleDataById(item.id)">
...
export default {
    ...
    methods: {
        ...
        async deleteExampleDataById(exampleId) {
            const authStore = useAuthStore()
            const token = authStore.accessToken
            try {
                const config = useRuntimeConfig()
                const response = await fetch(`${config.public.baseURL}/project/dataset/${this.projectId}/examples/${exampleId}`, {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${token}`
                ...
                if (!response.ok) {
                    ...
                    throw new Error(errorData.message || 'Failed to delete example data');
                }
        ...
        async editText(example_id, uuid) {
            navigateTo(`/project/${this.projectId}/${example_id}/edit?uuid=${uuid}`)
        },
